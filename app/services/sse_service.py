...
_tickets: dict[str, tuple[uuid.UUID, datetime]] = {}
...
def issue_ticket(current_user: User) -> str:
    ...
    _tickets[hash_token(raw_token)] = (current_user.village_id, expire_at)
    return raw_token


def resolve_ticket(raw_token: str) -> uuid.UUID:
...
def subscribe(village_id: uuid.UUID) -> asyncio.Queue:
    queue: asyncio.Queue = asyncio.Queue()
    _subscribers[village_id].add(queue)
    return queue


def unsubscribe(village_id: uuid.UUID, queue: asyncio.Queue) -> None:
